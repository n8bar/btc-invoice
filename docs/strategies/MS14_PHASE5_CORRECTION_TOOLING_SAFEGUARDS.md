...
## Verify Phase 5
### Automated
- [x] Reattribution recomputes source and destination invoices truthfully.
- [x] Same-owner destination validation holds.
- [x] `invoice_payments` keeps immutable source provenance while only the active accounting destination and current reattribution state change.
- [x] Reattribution audit logs and metadata persist.
- [x] Reattribution suppresses queued payment-triggered deliveries that would become untruthful.
- [x] Stale-address wrong-invoice cases stay outside unsupported-wallet evidence without separate facts.
- [x] Source and destination histories preserve the correct reattributed-out / reattributed-in presentation.
- [x] Owner/support related-invoice links stay off public/print surfaces.
- [x] Force delete stays blocked across all unresolved bookkeeping blocker classes.
- [x] `./vendor/bin/sail artisan test`
