...
2. [x] Confirm the controlled MS14 baseline from [`docs/strategies/14.1_BASELINE_RESEED.md`](14.1_BASELINE_RESEED.md) is present. If invoices `50` through `58` are missing, stop and rebuild that baseline first.
...
Current next-step/fix tracking during/after Browser QA lives in [`docs/strategies/14.5_BQA_FINDINGS_AND_FIXES.md`](14.5_BQA_FINDINGS_AND_FIXES.md).
