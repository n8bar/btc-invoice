...
13. **UX Overhaul**
    ...
    - [ ] Invoices & Clients UI polish (ToDo follow-up): complete carryover UX polish for authenticated invoice/client surfaces (narrower-screen sanity + action wrapping only), then merge it back into the completed Task 9 record in [`docs/UX_OVERHAUL_SPEC.md`](UX_OVERHAUL_SPEC.md).
    - [ ] Public/share layout refresh to mirror updated show/print patterns; friendly disabled/expired states.
      - Sequencing note (2026-02-21): Task 10 implementation subitems remain tracked in ToDo with completed checks preserved; public/share narrower-screen verification stays in Task 10, and final Task 10 acceptance is gated on closing Task 9 (ToDo) first.
