...
Latest scope update (2026-03-05): no new post-MVP backlog items in this pass; active PLAN/UX spec refined MS13 Task13 scope wording (invoice settings + invoice UX finish-up), kept the invoice-create simplification (new invoices always start as draft, status set after creation), added client-email hardening (required on create/edit + `clients.email` non-null schema target), and queued three additional in-scope Task13 polish items (paid-print watermark expansion, biller-branded client-facing over/underpayment wording, and hiding QR/BIP21 payment actions once invoices are paid across owner/client surfaces), while this doc remains focused on deferred post-MVP items such as invoices/clients search+filter and multi-wallet selection UX.
