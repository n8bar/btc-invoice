...
Latest scope update (2026-03-05): no new post-MVP backlog items in this pass; active PLAN/UX spec refined MS13 Task13 with an invoice-create simplification (new invoices always start as draft, status set after creation), while this doc remains focused on deferred post-MVP items such as invoices/clients search+filter and multi-wallet selection UX.
