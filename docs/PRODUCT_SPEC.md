...
- Mailer and alerts hardening must add duplicate-send safeguards, including intent-level and send-boundary idempotency, support owner-editable templates with safe variables and preview/reset flows, and tighten queue and delivery safety around outbound email, including owner validation gates for later payment-triggered mail on already-funded invoices.
