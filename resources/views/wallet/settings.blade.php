            ...
            <div class="overflow-hidden bg-white shadow-sm sm:rounded-lg">
	                <div class="p-6 text-gray-900">
	                    <div class="mb-4 space-y-2 text-sm text-gray-600">
	                        <p>{{ __('Provide a BIP84 xpub/zpub so a unique Bitcoin address is generated per invoice.') }}</p>
                    ...
                    <form method="POST" action="{{ route('wallet.settings.update') }}" class="space-y-6">
	                        ...
	                        @if ($defaultNetwork !== 'mainnet')
	                            <div class="rounded border border-blue-200 bg-blue-50 px-3 py-2 text-sm text-blue-800">
	                                {{ __('Testnet (for testing only). Real payments require mainnet. Use a testnet wallet key that starts with vpub or tpub.') }}
	                            </div>
	                        @endif

	                        <div>
	                            <x-input-label for="bip84_xpub" :value="$defaultNetwork === 'mainnet' ? __('BIP84 xpub/zpub') : __('BIP84 vpub/tpub')" />
